self: super: let
  ...
  mkOptimisedPipewire = pgoMode: let
    fdk_aac = (super.fdk_aac.override {stdenv = makeStatic stdenv;}).overrideAttrs (autotoolsOptions_pgo (getDrvName self.pipewire-optimised) pgoMode "instr" (old: {
      configureFlags = (old.configureFlags or []) ++ ["--with-pic"];
    }));
    pipewire-optimised =
      (super.pipewire.override {inherit stdenv fdk_aac;}).overrideAttrs (mesonOptions_pgo (getDrvName self.pipewire-optimised) pgoMode "instr" fakeExtra);
